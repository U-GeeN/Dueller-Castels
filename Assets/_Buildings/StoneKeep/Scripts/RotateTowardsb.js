...
function Action() {
	...
	if(opened == false) {
		...
		while(transform.rotation.eulerAngles != newRot) {
			//rotate door
            transform.rotation.SetFromToRotation( transform.rotation.eulerAngles, newRot );
			//transform.rotation.eulerAngles = Vector3.RotateTowards(transform.rotation.eulerAngles, newRot, Time.deltaTime * rotateSpeed, 0.0f);
